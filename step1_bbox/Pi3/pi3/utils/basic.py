...
def load_images_as_tensor(path='data/truck', interval=1, PIXEL_LIMIT=255000):
    ...
    actual_size = min(W_orig, H_orig)
    first_img.resize((actual_size, actual_size), Image.Resampling.LANCZOS)
    scale = math.sqrt(PIXEL_LIMIT / (actual_size * actual_size)) if actual_size * actual_size > 0 else 1
    W_target, H_target = actual_size * scale, actual_size * scale
