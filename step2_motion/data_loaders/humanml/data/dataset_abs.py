...
class Text2MotionDatasetV2(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        filename = self.name_list[data_idx]
        ...
        # randomly set to empty for classifier free guidance
        if not self.valtest and random.random() < 0.1:
            caption = ''
            tokens = []
            abstraction = np.zeros_like(abstraction)
        ...
        return word_embeddings, pos_one_hots, caption, sent_len, motion, m_length, '_'.join(tokens), abstraction, self.max_abs_num, filename, position
