...
def add_generate_options(parser):
    ...
    group.add_argument("--no_dataset", action='store_true', default=False,
                       help="Skip dataset loading for inference-only mode (requires --text_prompt and --hint_path)")
    ...
    group.add_argument("--cfg_scale", default=1.0, type=float,
                       help="CFG scale for classifier-free guidance.")

def add_hint_options(parser):
    group = parser.add_argument_group('hint_normalization')
    group.add_argument("--torso_idx", default=3, type=int,
                       help="Index of the torso cube for xz-centering")
    group.add_argument("--target_height", default=1.3, type=float,
                       help="Target height for uniform scaling")
    group.add_argument("--y_offset", default=0.3, type=float,
                       help="Vertical offset after normalization")
    group.add_argument("--camera_rot_x", default=-10.0, type=float,
                       help="Camera correction rotation around X (degrees)")
    group.add_argument("--align_direction", action='store_true', default=True)
    group.add_argument("--no_align_direction", dest='align_direction',
                       action='store_false')
    group.add_argument("--rotation_idx", default=-1, type=int,
                       help="Fix rotation to this variant index (0-based). "
                            "All batch samples use the same rotation. -1 = sweep mode.")
    group.add_argument("--num_rotations", default=10, type=int,
                       help="Total number of rotation divisions (used to compute angle from rotation_idx)")
    group.add_argument("--vis_bbox", action='store_true', default=True,
                       help="Visualize bounding-box hints in the output video")
    group.add_argument("--no_vis_bbox", dest='vis_bbox', action='store_false')
